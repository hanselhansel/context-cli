...
def test_quiet_backwards_compat_default_threshold():
    ...
    with patch("aeo_cli.main.audit_url", side_effect=_fake_audit_url_low):
...
def test_fail_on_blocked_bots_exits_2():
    ...
    with patch("aeo_cli.main.audit_url", side_effect=_fake_audit_url_blocked):
        result = runner.invoke(
            app,
...
def test_blocked_bots_priority_over_score_failure():
    ...
    with patch("aeo_cli.main.audit_url", side_effect=_fake_audit_url_low_blocked):
        result = runner.invoke(
            app,
            [
                "audit", "https://example.com", "--single",
...
def test_fail_under_with_site_audit():
    ...
    with patch("aeo_cli.main.audit_site", side_effect=_fake_audit_site_low):
        result = runner.invoke(
            app,
