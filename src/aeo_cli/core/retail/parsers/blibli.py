"""Blibli.com product page parser."""
...
class BlibliParser(BaseParser):
    """Parse Blibli.com product listing pages."""

    def parse(self, html: str) -> ProductData:
        """Parse Blibli HTML into ProductData."""
        ...
        qa_count = self._get_qa_count(soup)
        ...
        return ProductData(
            ...
            qa_count=qa_count,
            schema_org=schema,
            marketplace=MarketplaceType.BLIBLI,
            alt_texts=alt_texts,
        )
    ...
    def _get_description(self, soup: BeautifulSoup, schema: dict) -> str | None:
        ...
        el = soup.find(attrs={"data-testid": "lblPDPDetailProductDescription"})
        if el:
            return el.get_text(strip=True)
        return None
    ...
    def _get_brand(self, soup: BeautifulSoup, schema: dict) -> str | None:
        ...
        if isinstance(brand, str):
            return brand
        el = soup.find(attrs={"data-testid": "lblPDPDetailBrandName"})
        if el:
            return el.get_text(strip=True)
        return None
    ...
    def _get_rating(
        ...
        if agg.get("reviewCount"):
            try:
                ...
                pass
        if rating is None:
            el = soup.find(attrs={"data-testid": "lblPDPDetailProductRating"})
            if el:
                try:
        ...
        if review_count is None:
            el = soup.find(attrs={"data-testid": "lblPDPDetailReviewCount"})
            if el:
    ...
    def _get_specifications(self, soup: BeautifulSoup) -> dict[str, str]:
        ...
        specs: dict[str, str] = {}
        for row in soup.find_all(class_="spec-row"):
            key = row.find(class_="spec-key")
            val = row.find(class_="spec-val")
            if key and val:
    ...
    def _get_qa_count(self, soup: BeautifulSoup) -> int | None:
        """Extract Q&A count."""
        qa_section = soup.find(class_="qa-section")
        if qa_section:
            items = qa_section.find_all(class_="qa-item")
