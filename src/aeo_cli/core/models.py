...
class ProductData(BaseModel):
    ...
    price: str | None = Field(
        default=None, description="Product price as displayed"
    )
    ...
    rating: float | None = Field(
        default=None, description="Average rating (0-5)"
    )
    ...
    has_video: bool = Field(
        default=False, description="Whether listing has video content"
    )
    has_aplus_content: bool = Field(
        default=False, description="Whether listing has A+ content"
    )
    qa_count: int | None = Field(
        default=None, description="Number of Q&A entries"
    )
