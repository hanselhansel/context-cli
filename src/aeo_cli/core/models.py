...
class MarketplaceType(str, Enum):
    """Supported marketplace types for retail audit."""
...
class ProductData(BaseModel):
    """Parsed product data extracted from a marketplace listing."""

    title: str | None = Field(default=None, description="Product title")
    description: str | None = Field(
        default=None, description="Product description text"
    )
...
class ProductSchemaReport(BaseModel):
    """Product schema pillar report (max 25 points)."""

    score: float = Field(default=0.0, description="Product schema pillar score (0-25)")
    has_product_schema: bool = Field(
        default=False, description="Whether Product JSON-LD exists"
    )
    has_offer: bool = Field(default=False, description="Whether Offer data exists")
    has_aggregate_rating: bool = Field(
        default=False, description="Whether AggregateRating exists"
    )
    missing_fields: list[str] = Field(
        default_factory=list, description="Missing schema fields"
    )


class ContentQualityReport(BaseModel):
    """Content quality pillar report (max 30 points)."""

    score: float = Field(default=0.0, description="Content quality pillar score (0-30)")
    bullet_count: int = Field(default=0, description="Number of bullet points")
    description_length: int = Field(default=0, description="Description character count")
    has_aplus: bool = Field(default=False, description="Has A+/enhanced content")
    has_spec_chart: bool = Field(default=False, description="Has specification chart")


class VisualAssetsReport(BaseModel):
    """Visual assets pillar report (max 15 points)."""

    score: float = Field(default=0.0, description="Visual assets pillar score (0-15)")
    image_count: int = Field(default=0, description="Number of product images")
    images_with_alt: int = Field(default=0, description="Images with alt text")
    has_video: bool = Field(default=False, description="Has video content")


class SocialProofReport(BaseModel):
    """Social proof pillar report (max 20 points)."""

    score: float = Field(default=0.0, description="Social proof pillar score (0-20)")
    review_count: int | None = Field(default=None, description="Number of reviews")
    rating: float | None = Field(default=None, description="Average rating")
    has_qa: bool = Field(default=False, description="Has Q&A section")


class FeedComplianceReport(BaseModel):
    """Feed compliance pillar report (max 10 points)."""

    score: float = Field(default=0.0, description="Feed compliance pillar score (0-10)")
...
class RetailAuditReport(BaseModel):
    """Complete retail audit report with per-pillar breakdown."""

    url: str = Field(description="Audited product URL")
    marketplace: MarketplaceType = Field(description="Detected marketplace")
    score: float = Field(default=0.0, description="Overall retail score (0-100)")
    product_schema: ProductSchemaReport = Field(
        default_factory=ProductSchemaReport, description="Product schema analysis"
    )
    content_quality: ContentQualityReport = Field(
        default_factory=ContentQualityReport, description="Content quality analysis"
    )
    visual_assets: VisualAssetsReport = Field(
        default_factory=VisualAssetsReport, description="Visual assets analysis"
    )
    social_proof: SocialProofReport = Field(
        default_factory=SocialProofReport, description="Social proof analysis"
    )
    feed_compliance: FeedComplianceReport = Field(
        default_factory=FeedComplianceReport, description="Feed compliance analysis"
    )
    product_data: ProductData = Field(
        default_factory=ProductData, description="Parsed product data"
    )
    errors: list[str] = Field(
