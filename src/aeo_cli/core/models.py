...
class CitationSource(BaseModel):
    ...
    domain: str | None = Field(default=None, description="Domain of the source")
...
class BrandMention(BaseModel):
    ...
    count: int = Field(description="Number of times mentioned")
    sentiment: str = Field(default="neutral", description="positive, neutral, or negative")
    context_snippets: list[str] = Field(
        default_factory=list, description="Text around each mention"
    )
...
class DomainCategory(BaseModel):
    ...
    category: str = Field(
        description="Category: reddit, news, review_site, marketplace, blog, official, other"
    )
...
class RadarReport(BaseModel):
    ...
    domain_breakdown: list[DomainCategory] = Field(
        default_factory=list, description="Domain categorization"
    )
    total_citations: int = Field(
        default=0, description="Total citations across all models"
    )
